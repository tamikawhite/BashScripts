...
# Prompt the user to enter the fully qualified domain name (FQDN) of the worker node
read -p "Enter the fully qualified domain name (FQDN) for the worker node: " fqdn_worker

# Prompt the user to enter the IP address of the worker node
read -p "Enter the IP address for the worker node: " ip_address_worker

# Create the config.yaml file with the provided contents
cat <<EOF | sudo tee /etc/rancher/rke2/config.yaml > /dev/null
token: my-shared-secret
server: https://$fqdn_node1:9345
tls-san:
  - $fqdn_worker
  - $ip_address_worker
EOF
